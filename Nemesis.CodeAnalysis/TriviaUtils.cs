...
public static class TriviaUtils
{
    ...
    public static byte GetElementIndentationLevel(SyntaxNode node)
    {
        ...
        var hierarchy = GetHierarchy(node).Where(n => n is not CompilationUnitSyntax);
        return (byte)hierarchy.Count();
    }

    public static TNode NormalizeWhitespace<TNode>(TNode node, SyntaxNode parent, bool indentWithTabs = false)
        where TNode : SyntaxNode
    {
        parent ??= node.Parent ?? throw new ArgumentNullException(nameof(parent), $@"{nameof(parent)} has to be not null or {nameof(node)}.Parent needs to point to not null reference");
        ...
        var lastWhitespaceTrivia = parentLeadingTrivia.LastOrDefault(t => t.IsKind(SyntaxKind.WhitespaceTrivia));
        ...
        node = node.NormalizeWhitespace(indentWithTabs ? "\t" : "    ").WithTriviaFrom(node);

        return TriviaPrefixer.Prepend(node, baseIndentation);
    }

    public static TNode NormalizeWhitespace<TNode>(TNode node, int baseIndentationLevel, bool indentWithTabs = false)
        where TNode : SyntaxNode
    {
        var singleIndentation = indentWithTabs ? new[] { SF.Tab } : new[] { SF.Space, SF.Space, SF.Space, SF.Space };
