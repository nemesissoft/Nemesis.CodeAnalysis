...
namespace Nemesis.CodeAnalysis.Tests
{
    ...
    class TestClass
    {
        ...
        private const long NOTSET = -1;
        private const string strConst = ""Ala has a cat"";
        private long _mPreOpenSliceTimeEndTimeOffsetSecs = NOTSET;
        ...
        private static IEnumerable<TestCaseData> GetLocalVariables_ShouldHandleTypes_TestCases() => new (string varName, string expectedType)[]
            ...
            (tuple, i) => new TestCaseData(tuple.varName, tuple.expectedType)
        ...
        [Test]
        public void GetConstructors_ShouldYieldAllCtors()
        {
            var classSymbol = _model.GetDeclaredSymbol(_class);
            
            var ctors = SymbolUtils.GetInstanceConstructors(classSymbol);

            var @params = ctors.Select(c => string.Join(" ", c.Parameters.Select(p => p.Type.Name)));
