...
namespace MainNamespace
{
    class Program
    {
        ...
        {
            Type program = assembly.GetType("MainNamespace.Program");
            MethodInfo printMethod = program.GetMethod("Print") ?? throw new InvalidOperationException("No Print method");
            object programInstance = Activator.CreateInstance(program);
...
";
            ...
            if (result.Success)
            {
                ...
                var type = assembly.GetType(SCRIPT_CLASS);
                ...
                dynamic methodResult = method.Invoke(null, new object[] { new object[2] });
